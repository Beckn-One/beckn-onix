...
func (m *Manager) OndcWorkbench(ctx context.Context,cache definition.Cache,Km definition.KeyManager,cfg *Config) (definition.OndcWorkbench, error) {
	owp, err := provider[definition.OndcWorkbenchProvider](m.plugins, cfg.ID)
	if err != nil {
		return nil, fmt.Errorf("failed to load provider for %s: %w", cfg.ID, err)
	}
	ow, closer, err := owp.New(ctx, cache, Km, cfg.Config)
	if err != nil {
		return nil, err
	}
	if closer != nil {
